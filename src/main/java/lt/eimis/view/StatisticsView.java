...
public class StatisticsView implements Serializable {
	...
	public void prepareStatistics() {
		...
		for (int i = 0; i < sortedStats.size(); i++) {
			int begin = i;
			int end = i;
			while (end < sortedStats.size() - 1
					&& sortedStats.get(end + 1).getSortKey() == sortedStats.get(i).getSortKey()) {
				end++;
			}
			while (begin > 0 && sortedStats.get(begin - 1).getSortKey() == sortedStats.get(i).getSortKey()) {
				begin--;
			}
			if (begin != end) {
